...
namespace TheGameOfLife
{
    public class TheGameBoard
    {
        ...
        public void NextDoorCell()
        {
            for (int y = 0; y < 30; y++)
            {
                for (int x = 0; x < 60; x++)
                {
                    ...
                    if (y != 0 && x != 59 && GameBoard[y - 1, x + 1].Life == true) { GameBoard[y, x]._nextDoor += 1; }
                    if (y != 0 && x != 0 && GameBoard[y - 1, x - 1].Life == true) { GameBoard[y, x]._nextDoor += 1; }
                    if (y != 29 && GameBoard[y + 1, x].Life == true) { GameBoard[y, x]._nextDoor += 1; }
                    if (y != 29 && x != 59 && GameBoard[y + 1, x + 1].Life == true) { GameBoard[y, x]._nextDoor += 1; }
                    if (y != 29 && x != 0 && GameBoard[y + 1, x - 1].Life == true) { GameBoard[y, x]._nextDoor  +=1; }
                    if (x != 59 && GameBoard[y, x + 1].Life == true) { GameBoard[y, x]._nextDoor += 1; }
        ...
        public void GliderGun20()
        {
            ...
            GameBoard[3, 48].Life = true;
            GameBoard[4, 48].Life = true;
            GameBoard[3, 49].Life = true;
            GameBoard[4, 49].Life = true;
        }
